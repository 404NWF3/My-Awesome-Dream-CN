...
#### 已落地 ▸ 初始化 / strategy 模板 / dashboard 落盘（grill 共识 2026-07-22）
...
#### 预设提示词清单（已落地 2026-07-22）

12 条预设提示词已录入 `wiki/meta/preset-prompts.md`，覆盖决策树全路径（grill-me-study 形态 A，根节点→路径甲/乙→合流 Q3-Q8→输出方向卡片），每条带 `preset_slug` 对应节点：

- 根：`competitiveness-self`（Q0 竞争力自评）
- 路径甲（挖动机）：`goal-clarify`（Q1）/ `research-direction`、`employment-gateway`、`habit-vs-motive`（Q2 三分支）
- 路径乙（盘牌）：`hand-audit`（Q1）/ `hand-direction`（Q2）
- 合流：`willing-cost`（Q3）/ `degree-target`（Q4）/ `geo-city`（Q5）/ `target-match`（Q6）/ `timeline-check`（Q7）/ `swot-gap`（Q8）
- 终：`direction-card`（产出方向卡片）

grill-me-study 自行判断当前节点、调用对应条目；用户作答后由 Claudian 回复沉淀进 `wiki/questions/`（`prompt_type: preset`），被 dashboard 第 4 区命中。

#### 全部落地 —— 开发计划已完成（grill 共识 + 实施 2026-07-22）

32 节共识全部落地为代码/文件/配置/脚本/模板/面板/清单。Plan 已实现，后续只需运行时可迭代调优。
...
#### 已落地 ▸ ingest 触发与去重合并（grill 共识 2026-07-22）
...
- **置信度与时效**：置信度同社区/官方分层聚合，展示时"社区归社区、官方分官方、最后综合联动"；过期招生通知在 concept/entity 层打时效标记（见下方「entity/concept 层来源追踪与时效」「时效三层与 lint」——全部已落地）。

#### 已落地 ▸ ingest 提炼粒度 + 置信度时效字段（grill 共识 2026-07-22）

- 一次 ingest 批上限 N=5-10，沿用官方默认 8-15 页粒度，按 official 优先排序（见上方「ingest 分批与优先级」）。
- 置信度与时效已落 frontmatter 具体字段：`evidence_tier` / `confidence` / `as_of` / `valid_until` / `时效标签`（见下方「entity/concept 层来源追踪与时效」「时效三层与 lint」）。
...
### 已落地 ▸ 决策树骨架 + 沉淀节奏 + 方向卡片（grill 共识 2026-07-22）

- 决策树完整骨架已落（起手→路径甲/乙→Q3/Q4 合流→Q5-Q8→方向卡片，见下方「决策树骨架」）。
- 即时沉淀节奏 R2（关键节点才沉淀：Q4/Q6/Q8/方向卡片），见下方「即时沉淀节奏」。
- 产物方向卡片 K1 split（定性结论进 user entity，规划内容进 `wiki/strategy/`），见下方「设计判断」。
