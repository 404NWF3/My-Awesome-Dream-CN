...
> **本 skill 铁律（项目约束）**：本 skill 的产物**一律只写 `.raw/community-info/wechat/`**，调用时通过 `-o .raw/community-info/wechat/<fingerprint>.json` 指定输出，**禁止直写 `wiki/`**。初步处理后由 ingest 进 `wiki/sources/`。见项目根 AGENTS.md「社区域铁律」。

> **项目环境**：本项目使用仓库级 uv 虚拟环境（`.venv/`）管理 Python 依赖。本技能基于 Node.js，Node 依赖需另行安装（`npm install -g cheerio`）。
