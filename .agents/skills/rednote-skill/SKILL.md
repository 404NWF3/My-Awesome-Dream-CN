...
> **本 skill 铁律（项目约束）**：本 skill 的产物**一律只写 `.raw/community-info/rednote/`**，**禁止直写 `wiki/`**。已暴露 `--output-dir`/`-o` 参数与 `REDNOTE_OUTPUT_DIR` 环境变量（见 `rednote/commands/report_cmd.py:_resolve_output_dir`），调用时传 `-o .raw/community-info/rednote/` 或设该环境变量强制写向 `.raw/community-info/rednote/`. 初步处理后再由 ingest 进 `wiki/sources/`. 见 AGENTS.md「社区域铁律」.
