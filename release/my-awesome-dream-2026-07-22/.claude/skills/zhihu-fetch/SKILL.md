...
> **本 skill 铁律（项目约束）**：本 skill 的产物**一律只写 `.raw/community-info/zhihu/`**，调用时传输出目录 `.raw/community-info/zhihu/`，**禁掉写 Obsidian vault 选项、禁止直写 `wiki/`**。初步处理后再由 ingest 进 `wiki/sources/`。见项目根 CLAUDE.md「社区域铁律」。
