...
def _resolve_output_dir(explicit: str | None) -> str:
    """输出目录优先级：CLI flag > REDNOTE_OUTPUT_DIR 环境变量 > config output.reports_dir > 'data/reports'."""
    if explicit:
        return explicit
    env = os.environ.get("REDNOTE_OUTPUT_DIR")
    if env:
        return env
    return load_config()["output"]["reports_dir"]
...
def daily(
    ...
    output_dir: str = typer.Option(
        None, "-o", "--output-dir",
        help="报告输出目录（默认 REDNOTE_OUTPUT_DIR 环境变量 或 config output.reports_dir）",
    ),
):
    """生成运营日报."""
    ...
    async def _run():
        ...
        try:
            ...
            path = render_daily_report(user_id, all_notes, output_dir=_resolve_output_dir(output_dir))
            print(f"✅ 日报已生成: {path}")
