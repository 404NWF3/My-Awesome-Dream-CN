...
- **知乎**：zhihu-fetch 需浏览器 Cookie 持久化。提示用户在自己的浏览器登录知乎后运行 `uv run python .claude/skills/zhihu-fetch/scripts/save_cookies.py`（或按该 skill 的 SKILL.md 最新指示操作）。
...
> 如果 config 中某个来源的入参为空（如未填知乎收藏夹 URL），Claude 会跳过该项并提示补填。
